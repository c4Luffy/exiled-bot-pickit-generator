...
def syntax_guide_lines() -> list[str]:
    ...
    return [
        ...
        "// UniqueName:",
        "// ----------",
        "// Matches specific unique items by their exact name. It is read AFTER",
        "// the # - the name is only known once the item has been identified, so",
        "// it can never gate the pickup half.",
        '// Example: [Type] == "Heavy Belt" && [Rarity] == "Unique" # [UniqueName] == "Headhunter" && [StashItem] == "true"',
        "//",
        "// Disenchanting leftovers: Exiled Bot's own default.ipd uses a name that",
        "// can never match, on purpose. The pickup half takes EVERY unique, the",
        "// name matches nothing so this rule stashes none of them, and any unique",
        "// no OTHER rule kept is left to disenchant for Chance Shards.",
        '// Example: [Rarity] == "Unique" # [UniqueName] == "PICK ME AND DISENCHANT ME" '
        '&& [StashItem] == "true"',
        "//",
        "// ItemTier:",
        "// --------",
