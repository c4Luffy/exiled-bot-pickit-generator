...
• What the bot picked up now shows item art, each item's current price, and what the haul was worth, with a button to expand past the top twelve.

• The chance outcome list labels the unique you are chancing FOR. It was already gold, but in most pools that is also the dearest outcome, so it read as "the biggest number is coloured".
...
• The release tool now points the website at the release it just cut. Nothing did, so the landing page sat on v4.42.0 while v4.42.4 was out — including all six Download buttons, which served that stale build.
...
Also in 4.41.29:
• The Economy hover card no longer covers a row's keep/copy buttons. The detail card followed the cursor toward the right edge of the table and could sit right on top of the keep/skip and copy buttons — worse on a scaled or high-DPI display, where a whole column of buttons went unclickable. The card now stops at the left edge of that action column, so the buttons stay clear while the card still shows everything it did before, hovered or pinned.
